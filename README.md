...
최근가지 성과 관리 sass 개발하였고 주로 aws환경 python으로 제품 유지보수 고도화하였습니다
제품을 만드는 메이커로써 고객 불편사항, 세일즈 미팅 후기를 참고하여 여러 아이디어를 시도하여 제품을 개선하였습니다
또한 내부 도구를 만들어 동료, 팀의 업무 효율성을 높였습니다

제품을 만들면서 기술 영역을 구분하지 않고 문제 해결에 도움이 된다면
데이터 수집, 전처리, 분석, 시각화, 자동화 작업도 진행하며 경력을 쌓아왔고
백엔드 엔지니어로서는 시스템 안정성, 최적화를 담당하며 경력을 쌓아왔습니
</details>
...
소비자로서 크림을 사용할때 현재 트렌드 상품이 무엇인지 가격형성은 어떤지 알아보는 재미가 있었습니다
그런 와중 파트너 서비스 채용이 열렸고 백오피스의 업무 자동화, 대량 데이터 처리등 제 경력과 맞는 부분이 잇다 생각하여 지원하였습니다
</details>
...
제품을 만드는 큰틀에서 문제를 이해하고 근본적인 문제를 찾고 같이 논의할수 잇어야 하고
엔지니어로서는 코드를 잘 작성하는것은 물론 현재 리소스 상황, 로드맵을 고려한 합리적인 문제 해결 방안을 제시할 수 있어야 한다고 생각합니다
또한 구성원, 동료로서는 투명한 커뮤니케이션으로 작업 상황, 의견을 적극적으로 게시하는 개발자 좋은 개발자라 생각합니다
</details>
...
개인별 리포트 분석 기능에서 결과를 정적파일로 만드는 프로젝트 진행시
정적 파일 빌드 서버를 개발 vs 정적 파일 빌드 워커로 구현할지 의견이 나뉘었음

팀내에서도 의견이 나뉘었고 각 선택지별 장단점, 필요 리소스, 예상 작업일수 등을 파악하여
논의하여 현재 리소스 및 현실적인 부분을 감안하여 선택했음
...
분석 과정을 설계하는 것부터 분석 시각화까지 팀원들과 논의하는 과정에서 
제품을 만든다는 느낌일 받았습니다
시각화 부분에서 디자이너, 프론트 엔지니어와 같이 논의하고 데이터 시각화 방안을 서로 제시하고 테스트하며 더 좋은 결과를 만들기 위한 과정이 재미있었습니다
...
다형성으로 같은 이름의 메서드나 연산자가 다른 클래스에 대해 다른 동작을 하도록 하는 것을 말합니다.
...
원격지 서버의 기능을 마치 해당 서버의 함수처럼 사용하는 기술
grpc를 주로 사용하며 protbuf라는 무법으로 기능을 작성한다
grpc는 http2을 사용하여 데이터 전송 속도가 매우 빠르다
</details> 
...
@pytest.mark. 등을 사용하여 구분 가능
</details> 

<details> 
<summary>오버셀링 방지 대안 </summary>

우선 오버스톡 정책을 적용하여 여유 재고를 확보하도록 하여 문제 상황에 최소한 대응할수있도록한다
그리고 결제 단계에 진입하면 상품을 구매할수있는지 체크하고
구매하려는 상품 개수를 임시 예약하도록한다 레디스등으로 캐시성격으로 저장한다
상품 id가 키가되고 예약중인 상품수가 값이 된다 동시성 제어를 위해 Incrby 를 사용한다
더불어 어떤 사용자가 결제 단계에서 예약했는지 예약자이름과 상품, 상품 수를 캐시에 저장한다
결제가 완료되면 상품 예약 수량 캐시에서 예약 수량을 차감한다
더불어 사용자의 임시 예약 정보는 삭제한다
결제에서 이탈될경우 계속 예약으로 남길수 없기에 사용자 예약 정보는 ttl을 적용한다
키보드 마우스 이벤트를 감지하여 서버에 전송하고 ttl을 연장한다
ttl이 만료되면 상품의 예약 수량을 차감한다
위 흐름으로 결제 화면 이탈에 대응 가능하고
결제 취소시에도 동일한 흐름으로 대응 가능하다
</details> 
