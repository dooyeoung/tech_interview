...
최근 한국 주거 시장으로 보면 
전세사기 문제, 갭투자로 인한 보증금 반환 문제,
높은 월세에 비해 그렇지 못한 주거 품질 문제
개인이 감당해야할 리스크가 지나치게 크다고 생각합니다

또한 입주 후 유지보수에 대한 책임 구분 문제와 세탁기, 내장고 등 기기 관리 부담등 반복적으로 발생하는 문제들이 임차인에게 전가되는 구조가 계속 문제로 남아 있다고 생각합니다

이런 문제들을 맹그로브에서 코리빙으로 풀고잇는점이 인상깊었습니다
계약, 입주, 거주, 커뮤니티, 시설관리, 보안 전 과정을 하나의 흐름으로 설계하고 기술로 표준화하여 사업을 확장하는 점이 기존 부동산과 명확한 차이라 생각했습니다

dx group에서 pms, 커뮤니티, 출입제어, iot 관리 등 하나의 플랫폼으로 통합하려는 방향에 큰 관심이 생겼습니다

업무 자동화, 성능 최적화, 데이터 처리, 분석 등 운영 환경에서 발생하는 문제를 해결하며 소비스를 고도화해 온 경험이 있고 이를 맹그로브 주거 플랫폼이 안정적으로 확장하는데 기여하고자 지원하였습니다
...
저의 장점은 실행력과 이를 뒷받침 하는 긍정적인 태도 입니다
작업시 더 나은 방향, 여러 방법을 찾고 제시하고 시도하려합니다
할수 있다, 한번 해보자 라는 말을 의도적으로 더 많이 하며
생각하는 바를 실행에 옮겨 적용해고 눈으로 보고 개선하여 제품을 만들어갑니다
...
- mysql은 읽기 중심에 유리하고 수평 스케일에 강점이 있어 대규모 트래픽 대응에 많이 활용된다.
...
- timestamp는 1970-01-01 부터 사용,전 세계 사용자/서버 환경에서 동일한 시점을 기록할 수 있어 글로벌 서비스에 유리합니다.
조회하는 세션의 Timezone 설정에 따라 결과가 달라 보일 수 있습니다.

- 데이트 타임은 입력된 내용 그대로 저장(문자열처럼), 서버 타임존과 무관하게 변환 없이 저장됨
서비스가 여러 Timezone에 걸쳐 있다면, 각 데이터가 어떤 시간대의 시간인지 개발자가 별도로 기록하거나 알고 있어야 합니다.
...
- URL을 파싱한다.
- DNS로 도메인의 IP를 찾는다.
- 서버와 TCP 연결을 맺는다. syn syn + ack ack
...
- ssl후속기능, https:// 가 tls를 사용하는 프로토콜
...
일반 HTTPS는 서버만 인증하지만, mTLS는 클라이언트도 인증서를 제출해서 서버가 클라이언트의 신원을 검증합니다.

mTLS는 서버 ↔ 클라이언트 서로 인증함.
서버도 클라이언트의 신원을 확인하고, 클라이언트도 서버를 확인하는 양방향 인증 구조.
서버-서버 통신, 외부 파트너 연동, 내부 서비스 간 보안 강화에서 중요함.

파트너 API나 서비스 간 내부 통신에서 중요한 이유는,
API Key가 유출돼도 인증서가 없는 클라이언트는 연결 자체가 불가능하기 때문입니다.
즉, 요청 위변조는 HMAC으로 막고, 접근 주체 위조는 mTLS로 막는 구조입니다.
...
악성 스크립트를 웹 페이지에 주입하여, 그 페이지를 보는 다른 사용자(피해자)의 브라우저에서 실행되도록 합니다.

피해자의 세션 쿠키, 개인 정보 탈취, 또는 악성 콘텐츠 표시 등.
예시: 공격자가 게시판 댓글에 악성 `<script>` 태그를 포함하여 올리면, 다른 사용자가 그 댓글을 볼 때 스크립트가 실행되어 사용자의 쿠키가 공격자에게 전송됩니다.
...
사용자가 로그인된 상태를 악용하여, 공격자가 의도한 위조된 HTTP 요청을 피해자의 브라우저에서 서버로 보내도록 강제합니다.
피해자 모르게 비밀번호 변경, 송금, 계정 설정 변경 등 서버 측 액션 실행.

CSRF 토큰 사용, SameSite 쿠키 설정, Referer 헤더 확인.
</details> 
...
외부 API 호출이 연속적으로 실패하면 서킷 브레이커를 적용해 장애가 내부로 전파되는 것을 차단하겠습니다.
외부 API가 일정 횟수 이상 실패하면 회로가 Open 상태가 되어 외부 호출을 즉시 막고, 캐시·기본값 반환 등 fallback 로직만 실행해 전체 서비스는 정상적으로 유지합니다.
일정 시간이 지나면 Half-Open 상태에서 소량의 요청만 테스트하고 성공하면 Closed로 복구합니다.
서킷 Open 동안 처리하지 못한 작업은 Kafka 같은 메시지 큐로 비동기 보정 작업을 수행해 데이터 일관성을 유지합니다.
또한 타임아웃, 재시도 횟수 제한, Bulkhead 등을 함께 적용해 장애 영향 범위를 최소화합니다.
</details> 
...
<details> 
<summary>MSA & MA 차이 </summary>

MA는 하나의 애플리케이션 안에 모든 도메인과 기능이 포함된 구조로, 보통 하나의 코드베이스, 하나의 배포 단위를 가집니다.

장점
- 구조가 단순하고 초기 개발 속도가 빠름
- 디버깅과 로컬 테스트가 상대적으로 쉬움
- 트랜잭션 관리가 단순함 (단일 DB, 단일 프로세스)
- 배포 파이프라인과 운영 복잡도가 낮음

단점
- 기능과 팀 규모가 커질수록 코드 결합도 증가
- 특정 기능만 트래픽이 증가해도 전체를 같이 스케일해야 함
- 작은 수정에도 전체 빌드·배포 필요
- 장애 발생 시 영향 범위가 넓음


MSA는 도메인 또는 비즈니스 기능 단위로 서비스를 분리하여 각 서비스가 독립적인 배포·운영 단위를 가지는 구조입니다.

장점
- 서비스별 독립적 스케일링 가능
- 장애가 발생해도 전체 시스템으로 전파될 가능성 감소
- 팀 단위 개발 및 배포에 유리
- 기술 스택을 서비스별로 선택 가능

단점
- 서비스 간 통신 비용 증가 (네트워크, 지연)
- 데이터 정합성 관리가 어려움 (분산 트랜잭션)
- 모니터링, 배포, 장애 대응 등 운영 복잡도 증가
- 잘못 나누면 오히려 개발 속도 저하
</details>
...
Kafka는 여러 서비스가 동일 이벤트를 구독해야 하는 경우,
그리고 이벤트 자체를 로그처럼 일정 시간 보관하면서 재처리·재구독이 필요한 경우 선택합니다.
예를 들어 주문이 발생하면 알림, 추천, 재고 서비스가 각각 별도로 처리가능합니다

Kafka는 pull 기반이고, 파티션을 통한 수평 확장, 높은 처리량, 내구성 있는 로그 저장이 장점입니다.
대규모 트래픽이나 스트림 처리 적합합니다.
...
RabbitMQ는 짧은 지연과 즉시 전달이 중요한 비동기 작업 처리에 적합합니다.
예를 들어 주문 후 알림 발송·세금계산서 발행처럼
“하나의 작업을 빠르게 백그라운드에서 처리하는” 케이스에서 선택합니다.
RabbitMQ는 push 기반, 라우팅·우선순위·TTL 등 메시지 제어 기능이 다양해
업무 프로세스성 태스크 처리에 강합니다.
...
데드레터 큐는 처리할 수 없는 메시지를 별도로 격리해 전체 메시지 소비가 멈추지 않도록 하는 구조입니다.
Kafka는 DLQ 기능이 없어, 컨슈머가 실패 메시지를 별도 DLQ 토픽에 직접 기록해야 합니다.
이때 실패 메시지를 정상 처리로 간주하고 offset을 커밋해야 파티션이 멈추지 않습니다.
RabbitMQ는 Dead-Letter Exchange를 기본 제공하여 TTL 만료나 nack 같은 조건에서 메시지를 자동으로 DLQ 큐로 라우팅합니다.
이렇게 분리된 메시지는 전담 컨슈머가 재처리 또는 알림을 수행합니다.
</details>
...
“파트너센터처럼 외부와 맞닿는 백오피스 API는 Zero-trust 원칙을 그대로 적용합니다.
우선 내부망 여부와 상관없이 모든 요청을 불신하고 API Gateway에서 강제 인증·인가를 수행합니다.
API Key + Secret + HMAC으로 요청 위변조를 검증하고, 통신 자체는 mTLS를 통해 클라이언트 신원을 다시 한 번 확인합니다.
권한은 Key마다 Scope 단위로 최소 권한만 부여하고, Gateway에서 IP 제한, 속도 제한, 이상 패턴 탐지 같은 네트워크 정책도 함께 적용합니다.
마지막으로 모든 요청을 실시간 모니터링해 비정상 동작을 탐지하고, 키는 정기적으로 회전시켜 노출 위험을 최소화합니다.”
